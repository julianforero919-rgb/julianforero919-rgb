...
Ejecuta `npm ci` y luego `npm start` desde la raíz. En la base de datos ejecuta `db/Postgres.sql`, después `db/001_sedes_cargos.sql` y, si utilizarás contratos, `db/002_contratos.sql`. Comprueba `http://localhost:3000/api/health` y confirma `ok: true` antes de presentar.
