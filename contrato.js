...
let candidatoActualContrato = null;

const documento = {
  numero: document.getElementById('docNumeroContrato'),
  fecha: document.getElementById('docFechaContrato'),
  nombre: document.getElementById('docNombreTrabajador'),
  documento: document.getElementById('docDocumentoTrabajador'),
  cargo: document.getElementById('docCargoTrabajador'),
  inicio: document.getElementById('docFechaInicio'),
  fin: document.getElementById('docFechaFin'),
  salario: document.getElementById('docSalario'),
  sede: document.getElementById('docSedeTrabajador'),
  firma: document.getElementById('docFirmaTrabajador'),
  firmaDocumento: document.getElementById('docFirmaDocumento')
};
...
function actualizarDocumento() {
  if (!candidatoActualContrato) return;

  const salarioActual = Number(salario.value || 0).toLocaleString('es-CO');
  documento.fecha.textContent = new Date().toLocaleDateString('es-CO');
  documento.nombre.textContent = candidatoActualContrato.nombre_completo || '—';
  documento.documento.textContent = candidatoActualContrato.numero_documento || '—';
  documento.cargo.textContent = candidatoActualContrato.cargo || '—';
  documento.inicio.textContent = inicio.value || '—';
  documento.fin.textContent = fin.value || 'No definida';
  documento.salario.textContent = `$${salarioActual} COP`;
  documento.sede.textContent = candidatoActualContrato.nombre_sede || '—';
  documento.firma.textContent = candidatoActualContrato.nombre_completo || 'El Trabajador';
  documento.firmaDocumento.textContent = candidatoActualContrato.numero_documento || '—';
}
...
async function cargarPostulacionSeleccionada() {
  ...
  candidatoActualContrato = candidato;
...
[inicio, fin, salario, tipo, observaciones].forEach((campo) => {
  campo.addEventListener('input', actualizarDocumento);
  campo.addEventListener('change', actualizarDocumento);
});
