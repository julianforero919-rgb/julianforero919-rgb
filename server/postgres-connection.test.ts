...
describe.skipIf(!hasExplicitPostgresConfig)("SIDOVI schema foreign keys", () => {
  ...
  it("joins real cargo rows to Recursos Humanos and rejects an invalid owner", async () => {
    const client = await pool.connect();
    try {
      await client.query("BEGIN");
      const linked = await client.query(
        "SELECT c.id_cargo, c.id_rrhh FROM cargo c JOIN recursos_humanos r ON r.id_rrhh = c.id_rrhh ORDER BY c.id_cargo LIMIT 1",
      );
      expect(linked.rowCount).toBeGreaterThan(0);
      await expect(
        client.query(
          "INSERT INTO cargo (nombre_cargo, descripcion_cargo, turnos, id_rrhh) VALUES ($1, $2, $3, $4)",
